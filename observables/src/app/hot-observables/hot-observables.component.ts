...
import { Connectable, ConnectableObservable, Observable, Observer, Subject, publish, refCount, share } from 'rxjs';
...
export class HotObservablesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.myObservable = new Observable((observer: Observer<number>) => {
      let i: number = 0;
      console.log('%c Observable Created', 'backgorund: #cccccc; color: #ff0000')
      setInterval(()=> {
        i++;
  ...
  usingPublish(){
   //const multicasted = this.myObservable.pipe(publish(), refCount());
    const multicasted: ConnectableObservable<number> =
    this.myObservable.pipe(publish()) as ConnectableObservable<number>;
   //Subscriber 1
   setTimeout(() =>{
    multicasted.subscribe((_n) => {
      this.n1 = _n;
